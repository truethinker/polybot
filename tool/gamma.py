...
def gamma_list_markets_for_series_in_window(cfg: Config) -> list[dict]:
    """
    Pide a Gamma markets cuyo startDate cae dentro de la ventana (UTC),
    ...
    # Para BTC 5m, el slug suele ser "btc-updown-5m-<timestamp>"
    ...
    while True:
        ...
        # Si te vuelve a dar 422, aquí verás el body.
        if r.status_code >= 400:
            raise RuntimeError(f"Gamma error {r.status_code}: {r.text[:400]}")
